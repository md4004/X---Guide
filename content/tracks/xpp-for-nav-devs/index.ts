 ...
 *
 * The data half — frontmatter and tasks — lives in `lessons.ts`, which has no React
 * dependency so the engine's tests can import it. This file adds the compiled MDX
 * component for each lesson.
 *
 * Static imports rather than a filesystem glob: the bundler can see them, the types are
 * real, and a lesson that fails to compile breaks the build instead of 404ing at runtime.
 */
